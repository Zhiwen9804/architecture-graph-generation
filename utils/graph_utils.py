...
# -------- Mask batch of node features with 0-1 flags tensor -------- 用flags保留真正node部分，多余部分为0
def mask_x(x, flags):
...
# -------- Create flags tensor from graph dataset -------- 一个数组表示真正node的个数 例如有10个点 flags[10]为0
'''
...
''' 
def node_flags(graph_list, max_node_num = 40):
    flags = torch.zeros(len(graph_list), max_node_num).to(dtype=torch.float32)
    for i in range(len(graph_list)):
        node_num = graph_list[i].number_of_nodes()
        flags[i,:node_num] = torch.ones(node_num)
    return flags
...
def init_flags(graph_list, config, batch_size=None):
    ...
    idx = torch.randint(0, len(graph_list), (batch_size,))
...
def get_class_scaler(x):
    """Data normalizer. Assume data are always in [0, 14]."""
    # Rescale to [-1, 1]
    return ((x / 14.) * 2.) - 1.

def get_ratio_scaler(x):
    """Data normalizer. Assume data are always in [0, 1]."""
    # Rescale to [-1, 1]
    return x * 2. - 1.

def get_class_inverse_scaler(x):
    """Inverse data normalizer."""
    # Rescale [-1, 1] to [0, 14]
    return ((x + 1.) / 2.) * 14.
  
    
def get_ratio_inverse_scaler(x):
    """Inverse data normalizer."""
    # Rescale [-1, 1] to [0, 1]
    return (x + 1.) / 2.