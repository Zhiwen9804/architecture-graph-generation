...
class SDE(abc.ABC):
    ...
    def reverse_temb(self, score_fn, probability_flow=False):
        N = self.N
        T = self.T
        sde_fn = self.sde
        discretize_fn = self.discretize
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, feature, x, flags, temb, t, is_adj=True):
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                drift, diffusion = sde_fn(x, t) if is_adj else sde_fn(feature, t)
                score = score_fn(feature, x, flags, temb, t)
                drift = drift - diffusion[:, None, None] ** 2 * score * (0.5 if self.probability_flow else 1.)
                # -------- Set the diffusion function to zero for ODEs. --------
                diffusion = 0. if self.probability_flow else diffusion
                return drift, diffusion

            def discretize(self, feature, x, flags, temb, t, is_adj=True):
                """Create discretized iteration rules for the reverse diffusion sampler."""
                f, G = discretize_fn(x, t) if is_adj else discretize_fn(feature, t)
                score = score_fn(feature, x, flags, temb, t)
                rev_f = f - G[:, None, None] ** 2 * score * (0.5 if self.probability_flow else 1.)
...
class VESDE(SDE):
    ...
    def discretize(self, x, t):
        ...
        adjacent_sigma = torch.where(timestep == 0, torch.zeros_like(t, device=t.device), self.discrete_sigmas.to(t.device)[timestep - 1]).to(t.device)
