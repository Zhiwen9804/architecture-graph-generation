...
# -------- inpainter sampler --------
def get_pc_inpainter(sde_x, sde_adj, predictor='Euler', corrector='None', 
                   snr=0.1, scale_eps=1.0, n_steps=1, 
                   probability_flow=False, continuous=False,
                   denoise=True, eps=1e-3, device='cuda',
                   max_node_num=36, max_feat_num=2):

    def pc_inpainter(model_x, model_adj, init_flags, part_data):
        ...
        with torch.no_grad():
            ...
            for i in trange(0, (diff_steps), desc = '[Sampling]', position = 1, leave=False):
                ...
                masked_data_mean, std = sde_adj.marginal_prob(part_data, vec_t)
                masked_data = masked_data_mean + torch.randn_like(adj) * std[:, None, None]
                adj = adj * (1. - part_data) + masked_data * part_data
                adj_mean = adj_mean * (1. - part_data) + masked_data_mean * part_data
...
def get_pc_inpainter_plus(sde_x, sde_adj, predictor='Euler', corrector='None', 
    ...
    def pc_inpainter(model_x, model_adj, init_flags, part_x, part_adj, flags_x):
        shape_x = (init_flags.shape[0], max_node_num, max_feat_num)
        shape_adj = (init_flags.shape[0], max_node_num, max_node_num)
...
def S4_solver(sde_x, sde_adj, shape_x, shape_adj, 
                snr=0.1, scale_eps=1.0, n_steps=1, continuous=False,
                denoise=True, eps=1e-3, device='cuda'):
...
def load_sampling_fn(args):
    ...
    if args.predictor == 'S4':
        get_sampler = S4_solver
    else:
        get_sampler = get_pc_sampler
        
    shape_x = (args.batch_size, max_node_num, args.max_feat_num)
    shape_adj = (args.batch_size, max_node_num, max_node_num)
        
    sampling_fn = get_sampler(sde_x=sde_x, sde_adj=sde_adj, shape_x=shape_x, shape_adj=shape_adj, 
                                snr=0.15, scale_eps=0.7, 
                                n_steps=1, 
                                continuous=True, denoise=True, 
                                eps=1.0e-4, device=args.device)
    return sampling_fn